      ...
      <Row gutter={[32, 32]}>
        <Col xs={24} sm={8} md={6}>
          <Avatar
            src={
              people?.profile_path
                ? getMovieDetailImageUrl(people?.profile_path)
                : noImageUrl
            }
            size={300}
            shape="square"
                  ...
                  <Col xs={12} sm={8} md={6} lg={4} key={movie.id}>
                    <Link to={`/movie/${movie.id}`}>
                      <div style={{ textAlign: 'center' }}>
                        ...
                        <Title
                          level={5}
                          style={{ margin: '8px 0', fontSize: '1em' }}
                        >
                          {movie.title}
                        </Title>
                        {movie.character && (
            ...
            {people.movie_credits.cast.length > INITIAL_VISIBLE_ITEMS && (
              <div style={{ textAlign: 'center', marginTop: '20px' }}>
                <Button onClick={() => setShowAllCast(!showAllCast)}>
                  {showAllCast
                    ? 'Show Less'
                    : `View More (${people.movie_credits.cast.length - INITIAL_VISIBLE_ITEMS} more)`}
                </Button>
              </div>
            )}
          </div>
            ...
            {people.movie_credits.crew.length > INITIAL_VISIBLE_ITEMS && (
              <div style={{ textAlign: 'center', marginTop: '20px' }}>
                <Button onClick={() => setShowAllCrew(!showAllCrew)}>
                  {showAllCrew
                    ? 'Show Less'
                    : `View More (${people.movie_credits.crew.length - INITIAL_VISIBLE_ITEMS} more)`}
                </Button>
              </div>
            )}
          </div>
