...
import { useRemoveFavMovie } from '../../apis/user/useRemoveFavMovie';
...
const FavoriteMovies: React.FC = () => {
  ...
  const { removeFavMovie, loading: favoriteLoading } = useRemoveFavMovie();
  ...
  const handleRemoveFromFavorites = async (tmdbId: number) => {
    try {
      removeFavMovie(tmdbId.toString(), {
        onSuccess: () => {
          ...
          if (movies.length === 1 && currentPage > 1) {
            // If it is, go back to the previous page
